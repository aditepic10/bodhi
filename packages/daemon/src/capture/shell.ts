...
function buildJsonPayloadFunction(): string {
	return [
		...
		'      --argjson dur "$4" \\',
		'      --arg cwd "$5" \\',
		"      '{event_id:$eid,type:\"shell.command.executed\",metadata:{command:$cmd,exit_code:$exit,duration_ms:$dur,cwd:$cwd}}'",
		"    return",
		"  fi",
...
export function buildZshHookBlock(options: ShellHookOptions): string {
	return [
		...
		"  local payload",
		`  payload=$(__bodhi_payload "$eid" "${SHELL_COMMAND_SNIPPET}" "$exit_code" "${SHELL_DURATION_SNIPPET}" "$PWD") || { unset __bodhi_cmd; return; }`,
...
PY
		...
		'  eid=$(uuidgen 2>/dev/null) || eid="$(date +%s)-$$-$RANDOM"',
		"  local payload",
		`  payload=$(__bodhi_payload "$eid" "${SHELL_COMMAND_SNIPPET}" "$exit_code" "$duration" "$PWD") || { __bodhi_command_started=0; unset __bodhi_cmd; return; }`,
		'  if ! curl -s --max-time 0.1 --unix-socket "$__bodhi_sock" \\',
		"    -X POST http://localhost/events \\",
		"    -H 'Content-Type: application/json' \\",
		'    -d "$payload" >/dev/null 2>&1; then',
